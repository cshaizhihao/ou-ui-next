...
也可以直接运行最终现场验收快捷命令。`ou qf` 会生成证据包，并立即执行严格 `ou qv --require-runtime-evidence --require-browser-smoke --require-notification-smoke --require-webhook-smoke`；它会自动启用 runtime/通知/webhook 证据采集，禁止 `--skip-browser-smoke`，并要求显式提供 Telegram 测试目标：
