...
- **Service-backed HTTP control plane**
  ...
  - protected `/api/v1/observability-metrics` returns an operator diagnostics snapshot for task states, completion latency, rollback counts, command outbox backlog/leases/overdue/dead-letter counts, ACK/result latency, Agent offline/degraded counts, system-alert severity and kind counts including Agent offline, command outbox overdue/dead-letter, runtime reload failed, and quota exceeded alerts, system-alert webhook retry/dead-letter counts, retained Agent log chunk totals/bytes/time ranges, Agent log archive bucket/chunk/byte/time-range totals, retained traffic rollup totals, per-dimension counts, earliest/latest sample timestamps, cumulative metered bytes, audit-chain verification state, denied audit counts, and quota-exceeded audit counts
  ...
  - Agent telemetry now reports CPU, memory, disk, Linux load averages, latency bands, network traffic, and managed systemd service health for the Agent, Xray, and port-forwarding units; host-level quota or expiry guardrails disable managed Xray/port-forwarding units, later recover only units previously stopped by that guardrail and still present in the managed unit inventory, and report `hostGuardrailStoppedUnits` / `hostGuardrailRestoredUnits` evidence; managed-host details surface the load, latency, service-health, and liveness state, and Agent offline state, red high-latency samples, plus required service failures enter system alerts instead of requiring operators to inspect host files
  ...
  - subscription rules can filter nodes by protocol, region, source, managed host, runtime status, customer, and traffic conditions; the identity drawer exposes a dedicated Traffic Condition control that composes `traffic:*` rules, and local Xray nodes carry customer, host, status, used-traffic, and quota metadata for rule matching
