...
export type PublicSubscriptionFormat = SubscriptionProducerFormat;
...
function renderClash(
  nodes: SubscriptionInventoryNode[],
  client: SubscriptionClientIdentity,
  producerFormat: Extract<SubscriptionProducerFormat, 'clash' | 'mihomo' | 'stash'> = 'clash',
  exportProfile?: SubscriptionExportProfile
) {
  ...
  const banner =
    producerFormat === 'mihomo'
      ? '# mihomo-compatible subscription generated by OU-UI Next\n'
      : producerFormat === 'stash'
        ? '# stash-compatible subscription generated by OU-UI Next\n'
        : '';

  return `${banner}${toYaml(document)}\n`;
}
...
export function normalizePublicSubscriptionFormat(value: string): PublicSubscriptionFormat | undefined {
  return resolveSubscriptionOutputFormatAlias(value);
}
...
export function renderPublicSubscriptionOutput({
  ...
  const producerHeaders = producer
    ? {
        ...headers,
        'x-ou-ui-producer': producer.id,
        ...(producer.id === 'shadowrocket'
          ? {
              'profile-web-page-url': `https://ou-ui-next.local/sub/${projectedClient.subId}`
            }
          : {})
      }
    : headers;

  if (producer?.kind === 'base64-uri-list') {
    return {
      body: encodeBase64(uriBody),
      contentType: producer.contentType,
      headers: producerHeaders,
      nodeCount: nodes.length
    };
  }

  if (producer?.kind === 'clash-yaml') {
    const clashFormat = producer.format === 'stash' ? 'stash' : producer.format === 'mihomo' ? 'mihomo' : 'clash';

    return {
      body: renderClash(nodes, projectedClient, clashFormat, exportProfile),
      contentType: producer.contentType,
      headers: producerHeaders,
      nodeCount: nodes.length
    };
  ...
  if (producer?.kind === 'sing-box-json') {
    return {
  ...
  return {
    body: uriBody,
    contentType: producer?.contentType ?? 'text/plain; charset=utf-8',
    headers: producerHeaders,
    nodeCount: nodes.length
  };
}
