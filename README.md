...
- **服务化 HTTP Control Plane**
  ...
  - Agent 运行脚本每轮 poll 后上报 heartbeat，并默认每 30 秒采集 ping 延迟、CPU/内存/磁盘、系统负载、网络流量和受管 systemd 服务健康 telemetry；Master 短暂不可达时自动进入本地 pending 队列重试，队列默认最多保留 1000 条事件并优先保留 command ACK/result 等部署证据，受控主机详情会展示负载与 Agent/Xray/端口转发服务状态，离线、红色高延迟和必需服务异常会进入系统告警
