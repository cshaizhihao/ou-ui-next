...
export function createControlPlaneService({
  ...
  async function enqueueRuntimeHealthRollbackTask(
    ...
    const rollbackTask: DeployTask = {
      ...
      operatorGroupId: sourceTask.operatorGroupId,
  ...
  function createTaskStatusAudit(
    ...
    return {
      ...
      resourceGroupId: task.resourceGroupId,
      scope: `control-plane:${task.resourceType}`,
      resourceType: task.resourceType,
      operation: task.operation,
    ...
    async createTask(input: CreateTaskInput, context: MutationContext) {
      ...
      const result = await repository.transaction<CreateTaskTransactionResult>(async (transaction) => {
        ...
        if (task.operation === 'agent.rollback' && !readStringMetadata(task, 'snapshotId')) {
          const snapshotId = resolveRollbackSnapshotId(task, await transaction.listRuntimeSnapshots(), targetAgentIds);
