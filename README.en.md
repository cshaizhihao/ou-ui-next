...
To validate a real external archive provider, run the archive smoke test. Installed `ou as` reads `OU_UI_EXTERNAL_ARCHIVE_DIRECTORY`, `OU_UI_EXTERNAL_ARCHIVE_WEBHOOK_URL(S)`, and S3-compatible object-storage settings from the backend env by default, then writes one sanitized audit anchor, one Agent log archive summary, and one traffic compaction bucket to the configured sinks. This command performs real writes to the local archive directory, external archive webhooks, and object storage; reports do not include webhook tokens, object-storage credentials, full URL paths, or query strings:
