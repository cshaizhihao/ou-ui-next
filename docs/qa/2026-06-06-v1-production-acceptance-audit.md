...
- Agent-side `ou-agent doctor` output and `ou-agent qa` evidence bundle from that host, showing service state, runtime files, pending queue, event seq, Xray/GOST binary availability, guardrail state, redacted log tail, and SHA-256 manifest without printing runtime tokens.
