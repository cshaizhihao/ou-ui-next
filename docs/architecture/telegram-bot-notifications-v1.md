...
type TelegramBotSettings = {
  ...
  schedules: Array<{
    id: string;
    kind: 'traffic_threshold_scan' | 'expiry_scan' | 'subscription_update_scan' | 'system_alert_scan' | 'daily_report' | 'weekly_report' | 'delivery_retry';
...
- The background schedule scanner generates `traffic.threshold`, `subscription.expiring`, and `subscription.updated` delivery records from customer, subscription-user, Xray-client, forwarding-owner, and forwarding-rule bindings, and `system.alert` delivery records for configured administrator chat IDs. It does not call Telegram directly; the retry sweep sends the queued records through the same persisted delivery path as manual retries.
...
- Background proactive schedule scan wiring through `createServiceBackedControlPlane` for traffic threshold, expiry reminder, subscription update, and administrator system-alert delivery enqueueing with structured skip counts and dedupe keys.
...
- Scheduled proactive scans for provider sync warnings and daily/weekly reports.
...
- Dedupe key prevents repeated threshold, expiry, and subscription-output update notifications for the same billing window/output signature.
...
- Background schedule scan enqueues traffic threshold, expiry reminder, subscription update, and administrator system-alert notifications without direct Bot API calls; delivery retry sweep performs the actual send and preserves dedupe behavior.
