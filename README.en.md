...
You can also run the final field acceptance shortcut directly. `ou qf` writes the evidence bundle, then immediately runs strict `ou qv --require-runtime-evidence --require-browser-smoke --require-notification-smoke --require-webhook-smoke`; it automatically enables runtime, notification, and webhook evidence collection, forbids `--skip-browser-smoke`, and requires an explicit Telegram test target:
