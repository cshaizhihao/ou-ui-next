...
- Agent telemetry must persist host, port-forwarding, and Xray client counters into a protected, filterable, exportable, and retention-bounded traffic rollup history, not only update current snapshot counters; operators must be able to view the runtime default, control-plane override, and effective traffic rollup retention policy from the dashboard and save retention overrides through the protected API.
