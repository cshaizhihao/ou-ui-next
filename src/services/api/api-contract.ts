...
const xrayClientTaskMetadataSchema = z
  .object({
    ...
    trafficMultiplier: z.number().nonnegative().optional(),
...
const taskMetadataSchema = z
  .object({
    ...
    enabled: z.boolean().optional(),
    xrayGuardrailAutomatic: z.boolean().optional(),
    xrayGuardrailAction: z.enum(['disable', 'resume']).optional(),
    xrayGuardrailPolicyId: z.string().trim().min(1).max(255).optional(),
    xrayGuardrailPolicyScope: z.enum(['customer-node']).optional(),
    xrayGuardrailObservedAt: z.string().datetime().optional(),
    xrayGuardrailTriggerKind: z.enum(['agent-event', 'task']).optional(),
    xrayGuardrailTriggerId: z.string().trim().min(1).max(255).optional(),
    xrayGuardrailReason: z.string().trim().min(1).max(160).optional(),
...
export const operatorSessionRevokeRequestSchema = z.object({
  reason: z.string().trim().min(1).max(500)
});
...
export function parseOperatorSessionRevokeRequest(value: unknown): OperatorSessionRevokeRequestDto {
  const result = operatorSessionRevokeRequestSchema.safeParse(value);

  if (!result.success) {
    throw new Error(`Invalid operator session revoke request: ${result.error.issues.map((issue) => issue.path.join('.')).join(', ')}`);
  }
