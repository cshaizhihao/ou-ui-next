...
- **Service-backed HTTP control plane**
  ...
  - protected `/api/v1/observability-metrics` returns an operator diagnostics snapshot for task states, command outbox backlog/leases/overdue/dead-letter counts, Agent offline/degraded counts, system-alert severity counts, and audit-chain verification state
