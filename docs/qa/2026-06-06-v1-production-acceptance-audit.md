...
The script writes reports with `0600` permissions. Because it performs real writes to local archive directories, external archive webhooks, and S3-compatible object storage, it is exposed as a standalone `ou as` / `npm run smoke:archive` command and is not included in `ou qa` or `ou qf` by default.
...
- `npm run test -- production-archive-smoke-script install-master-script archive-sink audit-anchor-sink` - 4 files / 69 tests
...
- `npm run lint`
- `npm run test` - 62 files / 720 tests
- `npm run build`
- `git diff --check`
...
- External notification smoke output from operator-provided Telegram/webhook endpoints with real credentials. The repository now has reusable `ou ns` / `npm run smoke:notifications` tooling, `ou ws` / `npm run smoke:webhooks` webhook tooling, optional `ou qa --include-notification-smoke` Telegram bundle capture, optional `ou qa --include-webhook-smoke` webhook bundle capture, and `ou qvf` strict archive revalidation for final bundles, but this audit still lacks a real provider run proving delivery outside automated tests.
