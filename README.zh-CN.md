...
- **服务化 HTTP Control Plane**
  ...
  - Operator 会话会在服务端登记，可通过受保护的 `/api/v1/operator-sessions` 查看，并通过 `/api/v1/operator-sessions/{sessionId}/revoke` 精确撤销；`ou-ui doctor` 会报告 operator session 配置完整性、secret 是否存在和 TTL 健康，不输出 secret；撤销或退出登录后，原 session cookie 的后续受保护请求会被拒绝并写入审计链
