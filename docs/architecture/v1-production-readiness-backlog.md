...
- The SQLite repository still stores the current control-plane state as one transactional JSON document; production still needs a more normalized schema, HA replication strategy, explicit migrations, backup/restore policy, retention management, and stronger crash-recovery / storage controls.
