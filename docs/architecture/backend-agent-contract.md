...
- [ ] Agent 上报 ingress/egress counters，Master 聚合并标记采样 gap，保留可筛选/导出且受留存策略约束的 traffic rollup 历史，被留存策略剪枝的 raw 样本会压缩为可查询/导出的日级 compaction，并通过 observability/Prometheus 暴露 retained 存储压力。
