...
- **Service-backed HTTP control plane**
  ...
  - the Agent initializes its local `event-seq` file automatically before the first heartbeat, telemetry, ACK, or result event, so a fresh host cannot poll successfully while dropping runtime evidence because the sequence marker is absent
