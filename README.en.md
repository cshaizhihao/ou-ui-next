...
- **Service-backed HTTP control plane**
  ...
  - Agent telemetry now reports CPU, memory, disk, Linux load averages, network traffic, and managed systemd service health for the Agent, Xray, and port-forwarding units; managed-host details surface the load and service-health state instead of requiring operators to inspect host files
