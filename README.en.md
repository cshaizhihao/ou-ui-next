...
- **Service-backed HTTP control plane**
  ...
  - accepted managed-host delete tasks revoke every active runtime credential for that Agent in the same service transaction and append `agent.credential.revoked` audit evidence; deleted Agents cannot keep authenticating to poll/events with an old runtime token
  ...
  - Agent ACK/result/log events must match the command outbox record across `commandId`, `taskId`, and `agentId`; mismatched events return `agent_event.command_task_mismatch`, do not update Agent events or outbox state, and count as non-retryable `rejected` items in multi-event `/agent/v1/events` batches
