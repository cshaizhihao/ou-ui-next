...
- Operator diagnostics must expose a protected observability metrics snapshot covering task states, completion latency, rollback counts, command outbox backlog/lease/overdue/dead-letter counts, ACK/result latency, Agent offline/degraded counts, active alert severities, and audit-chain verification state.
