...
- The Xray production smoke failure path now preserves a redacted last-observed runtime evidence report with task, command, config revision, preflight, snapshot, read-model, and evidence-gap summaries, so real Agent failures or timeouts leave an actionable diagnosis instead of only a string error.
