...
- The execution workspace must expose retained Agent runtime log chunks with Agent, task, command, stream, and timestamp context so operators can inspect real runtime output without reading local state files.
