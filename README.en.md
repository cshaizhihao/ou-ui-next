...
- **Service-backed HTTP control plane**
  ...
  - external source sync only fetches `http` / `https` subscription URLs and supports per-source remote request timeouts plus response body limits; timeouts, oversize responses, and unsupported protocols become sync failure state and audit-chain entries
