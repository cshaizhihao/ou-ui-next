...
- `OU_UI_CONTROL_PLANE_STORAGE=sqlite`: persists the current control-plane repository state into `OU_UI_CONTROL_PLANE_SQLITE_FILE` inside a SQLite database file, enables WAL-backed transactional commits, validates `schema_version` / `state_format` metadata plus the `control_plane_migrations` ledger before serving reads or writes, backfills the current migration ledger for existing v1 stores, and can import a legacy JSON state file from `OU_UI_CONTROL_PLANE_LEGACY_STATE_FILE` when the database is first created.
...
- `OU_UI_EXTERNAL_ARCHIVE_WEBHOOK_URL` and `OU_UI_EXTERNAL_ARCHIVE_WEBHOOK_URLS`: optional external archive webhook targets for Agent log archive and traffic rollup compaction batches. Webhook delivery supports `OU_UI_EXTERNAL_ARCHIVE_WEBHOOK_TIMEOUT_MS`, `OU_UI_EXTERNAL_ARCHIVE_WEBHOOK_BEARER_TOKEN`, and `OU_UI_EXTERNAL_ARCHIVE_WEBHOOK_EGRESS_ALLOWLIST`; targets are deduped into labeled channels, block localhost/private/link-local/multicast destinations and resolved-private addresses, pin delivery to the verified public address while preserving Host / HTTPS SNI, and record sanitized structured delivery logs. When both a directory and webhook targets are configured, the runtime builds a composite archive sink; audit hash anchors remain file-backed through the archive directory.
- `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_ENDPOINT`, `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_BUCKET`, `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_REGION`, `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_ACCESS_KEY_ID`, and `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_SECRET_ACCESS_KEY`: optional S3-compatible object-storage archive sink. `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_PREFIX`, `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_SESSION_TOKEN`, `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_TIMEOUT_MS`, `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_FORCE_PATH_STYLE`, and `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_EGRESS_ALLOWLIST` control object keys, temporary credentials, request timeouts, addressing mode, and remote egress. `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_OBJECT_LOCK_MODE`, `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_OBJECT_LOCK_RETENTION_DAYS`, and `OU_UI_EXTERNAL_ARCHIVE_OBJECT_STORAGE_OBJECT_LOCK_LEGAL_HOLD` optionally add S3 Object Lock retention/legal-hold headers to the SigV4-signed PUT requests; mode and retention days must be configured together.

Production installs that use SQLite storage also expose `ou-ui backup-state` and `ou-ui restore-state <backup-path>` so operators can create local snapshots and restore them through the management CLI without hand-copying the database file. Backup commands now write a `.manifest.json` sidecar containing SHA-256, file size, storage mode, creation time, source commit, and the SQLite migration ledger; restore commands verify the manifest hash and size when present before staging the restored database, and legacy v1 backups get the current migration ledger on the restored target. SQLite backup, restore, validation, and `ou-ui doctor` diagnostics reject unsupported `schema_version`, `state_format`, or migration ledger metadata instead of accepting a database created by a newer OU-UI build. Installer-managed production also configures the file archive sink by default so retention-pruned Agent log summaries, traffic compaction buckets, and audit-chain anchors are retained outside the SQLite state payload.
...
- Service-backed Agent read models derive `online`, `degraded`, and `offline` status from the most recent heartbeat or telemetry signal using the configured 30-second probe cadence. Host telemetry snapshots separately derive sampling-gap state from `telemetry.reportedAt` and the Agent telemetry cadence, while red high-latency state still comes from the configured latency thresholds; fresh heartbeat events do not clear stale telemetry samples, and active Agent offline state, gaps, or high-latency samples project into the system alert read model.
- The published Agent runtime script sends heartbeat after each poll, samples ping/hardware/disk/network/traffic telemetry on the configured telemetry cadence that defaults to 1 second, queues automatic heartbeat/telemetry/log/ACK/result events for retry when Master delivery fails, bounds the local pending queue to 1000 events by default while pruning routine heartbeat/telemetry before command ACK/result evidence, rotates local `agent.log` at 5 MiB with 3 backups by default, emits bounded `log_chunk` events for command runtime output and result summaries after ACK and before result, executes explicit `health` and `telemetry` commands, and reports unsupported command types as failed results.
...
- Runtime apply health failures that trigger automatic rollback now derive `runtime.apply_health_failed` system-alert lifecycle records, notification events, dashboard labels, and observability/Prometheus kind counts. The alert is grouped by runtime target and resolves only after a newer successful Agent-result proof apply or rollback for the same target.
...
- The system-alert lifecycle derives runtime apply health failed and runtime reload failed alerts from the durable task read model; apply health alerts are grouped by runtime target and resolved only after a newer successful Agent-result proof apply or rollback for that same target, while reload alerts are resolved only after a newer successful reload for that same target.
