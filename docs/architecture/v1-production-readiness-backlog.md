...
- Service-backed external subscription sync now restricts remote fetches to `http` / `https`, applies request timeout and response body limits, and writes timeout/oversize/unsupported-protocol outcomes into the sync failure state and audit hash chain. Production still needs operator-configurable per-source fetch policy, network egress allowlists, and distributed fetch rate limiting.
