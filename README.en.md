...
- **Service-backed HTTP control plane**
  ...
  - the Agent initializes its local `event-seq` file automatically before the first heartbeat, telemetry, ACK, or result event; when host probe config is absent or `probeConfig` is `null`, telemetry falls back to the default ping target so a fresh host cannot poll successfully while dropping runtime evidence
  - Agent result events bound failure reasons to the backend contract length, 422 `validation_error` events are dropped as non-retryable with a local log entry, and port-forwarding nftables counter comments are quoted for nft syntax so one malformed result cannot permanently block the local pending queue
...
By default, `ou qv` verifies only the file sizes and SHA-256 hashes recorded in the manifest, keeping older bundles compatible. For field acceptance, add strict gates so the archived bundle must prove a valid UTC ISO `manifest.createdAt`, a non-empty `manifest.bundleDirectory`, main-manifest evidence file paths pointing at the current bundle or original bundleDirectory, valid schemaVersion plus valid UTC ISO `startedAt`/`completedAt` values in the HTTP/runtime, browser, notification, and webhook smoke reports, with `completedAt` not earlier than `startedAt`, non-empty `checks` with a non-empty `name`, valid UTC ISO `checkedAt` inside the `startedAt`/`completedAt` window, and `passed` status on every check in the HTTP/runtime, browser, and notification reports, plus runtime strict markers with `runtimeEvidenceRequired=true`, the runtime acceptance summary check marked `required=true`, a CSRF rejection probe proving `403 csrf.required`, a complete runtime acceptance summary with `audit.valid=true`, no exceeded or quota-disabled policies, and at least one Agent session advertising Xray or port-forwarding capability, runtime evidence, browser smoke, archived browser screenshots, notification smoke, and webhook smoke targets whose UTC ISO `checkedAt` values are inside the `startedAt`/`completedAt` window, whose HTTP status is 2xx, whose `responseBytes` is non-negative, passed, and sanitized. If the bundle was created with `ou qa --include-archive-smoke`, add `--require-archive-smoke` to require a valid `archive-smoke-report.json` schemaVersion, a valid UTC ISO `createdAt`, non-empty `checks` with a non-empty `name`, valid UTC ISO `checkedAt` not earlier than `createdAt`, and `passed` status on every check, and the real external archive delivery evidence as well. If the bundle carries provider-side receipt files, add `--require-external-receipts` to require a valid `external-receipts-manifest.json` `createdAt` and at least one receipt attachment with matching paths and hashes. If a receipt uses the `ou-ui-next.archive-provider-evidence.v1` sanitized JSON schema, add `--require-archive-provider-evidence` to require at least one receipt proving object-storage delivery plus provider-side Object Lock/retention policy. If a receipt uses the `ou-ui-next.timestamp-evidence.v1` sanitized JSON schema, add `--require-timestamp-evidence` to require at least one receipt proving a third-party timestamp receipt was sanitized, verified, and recorded with valid paths/hashes. If the bundle carries clean-server install evidence, add `--require-clean-install-evidence` to require a valid `install-evidence-manifest.json` `createdAt` and at least one install summary matching `ou-ui-next.clean-install-evidence.v1` with matching paths/hashes. If the bundle carries Agent host evidence, add `--require-agent-evidence` to require valid `createdAt` values on `agent-evidence-manifest.json` and the attached Agent manifest, at least one Agent bundle with a non-empty Agent `manifest.bundleDirectory`, `serviceStatus=0`, `runtimeSummaryStatus=0`, matching hashes, and a `runtime-summary.json` that proves Xray inbounds, port-forwarding services, an empty pending queue, and healthy guardrail evidence. To treat Agent host evidence as production release proof, add `--require-agent-final-summary`; it requires the Agent bundle to include `final-acceptance-summary.json` with a valid UTC `createdAt`, a non-empty `bundleDirectory` matching the Agent `manifest.bundleDirectory`, plus the verification transcript produced by `ou-agent qf`, and rechecks the manifest/transcript paths, sizes, and SHA-256 values recorded there. If the bundle was produced by `ou qf`, add `--require-final-summary` to recheck that the Master `final-acceptance-summary.json` has a valid UTC `createdAt`, a non-empty `bundleDirectory` matching `manifest.bundleDirectory`, plus the strict verifier transcript path/size/SHA-256. If the bundle was produced by `ou qfa`, add `--require-release-summary` to recheck that `release-acceptance-summary.json` has a valid UTC `createdAt`, a non-empty `bundleDirectory` matching `manifest.bundleDirectory` or the current bundle directory, the release verifier transcript path/size/SHA-256, and promote every release gate marker back into content verification for the current run:
...
- if the fresh install is not on the latest frontend, stale demo nodes still appear, shortcuts are missing, or the panel URL still returns Basic Auth, run `ou fix --force`; it updates from GitHub, rewrites the nginx panel site, then continues to clear old control-plane state, verify that the managed-host inventory is empty again, and check that the Agent install-command API is usable
- API calls are proxied through nginx; browser-side `/api`, `/events`, and `/metrics` requests pass an `auth_request` check against the HttpOnly session before nginx injects the backend operator token, while the exact public `/api/v1/boundary` route stays reachable through the secure path for version discovery. Session-backed `/api/v1` mutations also validate `X-CSRF-Token`. Neither the operator token nor the login password is written into the frontend bundle
