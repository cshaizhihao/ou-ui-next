...
- **服务化 HTTP Control Plane**
  ...
  - Agent ACK/result/log 事件的 `commandId`、`taskId` 和 `agentId` 必须同时匹配 command outbox 记录；错绑事件会返回 `agent_event.command_task_mismatch`，不会写入 Agent event、更新 outbox 或污染任务投影，批量 `/agent/v1/events` 中会作为不可重试项计入 `rejected`；所有会生成 Agent command 的 runtime 任务都不能通过人工 task transition 置为成功，必须由 Agent result 收敛
