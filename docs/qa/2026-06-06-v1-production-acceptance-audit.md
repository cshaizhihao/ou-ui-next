...
- `npm run test -- production-archive-smoke-script archive-sink audit-anchor-sink install-master-script` - 4 files / 72 tests
...
- `npm run lint`
- `npm run test` - 62 files / 723 tests
- `npm run build`
- `git diff --check`
...
- A final production acceptance run should attach `ou qf` command output plus `ou qvf` revalidation output or QA notes for each live deployment step above before marking V1 complete.
