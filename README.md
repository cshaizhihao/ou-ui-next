...
- **服务化 HTTP Control Plane**
  ...
  - Agent pending event 队列会继续重试网络/服务异常，但会丢弃 Master 明确返回的不可重试事件冲突（如 command deadline 已过或 event seq 已回放），避免旧事件永久阻塞后续 heartbeat、telemetry 和 result 上报；如果 ACK 被 Master 判定为过期命令，Agent 会跳过该命令而不是继续执行 stale runtime apply
