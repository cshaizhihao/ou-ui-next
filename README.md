...
- 客户节点删除失败时，runtime evidence 抽屉会优先聚焦最新 `inbound.delete` 任务、command/preflight/snapshot 证据和失败原因，并提供重试 runtime cleanup 入口，不会被旧的成功 apply proof 掩盖。
...
- 带有 `runtimeDisabledByPolicy`、`quotaExceeded` 或 `clientExpired` 证据的 client 会保留在 `clientPolicies[]`、配额视图和订阅诊断中，但不会进入实际 Xray `settings.clients`。
- 自动 guardrail 任务会按 client 派生 disable / resume intent；即使 read model 只带 `quotaExceeded` 或 `clientExpired` 而缺少显式 `runtimeDisabledByPolicy`，也会派生禁用任务并写回 guardrail reason。
...
| `OU_UI_AGENT_LOG_CHUNK_PERSIST_EVERY` | Agent 命令日志 chunk 持久化采样间隔，默认 `1` 表示全部保留；调高后仍会保留首个 chunk，并继续强持久化 ACK/result/config/preflight/snapshot runtime evidence |
