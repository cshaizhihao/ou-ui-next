...
- **Service-backed HTTP control plane**
  ...
  - protected `/events/v1/system-alerts` streams the current active system-alert snapshot and emits a new snapshot when the alert fingerprint changes; active alerts are now reconciled with a durable lifecycle read model and persist `active` / `resolved` evidence across restarts, while external notification fan-out remains a production-hardening item
