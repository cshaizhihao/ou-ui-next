...
prompt_agreement() {
  ...
  printf '%b' "${BOLD}${BLUE}"
  cat <<'EOF'
   ____  _   _       _   _ ___   _   _ _______
  / __ \| | | |     | | | |_ _| | \ | | ____\ \/ /_   _|
 | |  | | | | |_____| | | || |  |  \| |  _|  \  /  | |
 | |__| | |_| |_____| |_| || |  | |\  | |___ /  \  | |
  \____/ \___/       \___/|___| |_| \_|_____/_/\_\ |_|

                  OU-UI NEXT
          Master Control Plane Installer
EOF
  printf '%b\n' "${RESET}"

  cat <<'EOF'
╭────────────────────────────────────────────────────────────╮
│ 🚀 欢迎使用 OU-UI Next Master 一键安装器                    │
├────────────────────────────────────────────────────────────┤
│ 本脚本将部署自托管控制面板、HTTP Control Plane、Nginx 站点、 │
│ systemd 服务、SQLite 状态目录和管理命令。                    │
╰────────────────────────────────────────────────────────────╯

╭─ 📌 安装范围 ───────────────────────────────────────────────╮
│ 1. 自动安装或检查 Node.js、Nginx、cron、OpenSSL、jq 等依赖。  │
│ 2. 从 GitHub 拉取 OU-UI Next main 分支并在本机完成构建。      │
│ 3. 生成安全访问路径、操作员账号、操作员密码和后端 token。     │
│ 4. 写入 Nginx 配置、systemd 服务、SQLite 状态和静态资源。     │
│ 5. 如果你选择域名模式，会自动申请 Let's Encrypt 证书。        │
╰────────────────────────────────────────────────────────────╯

╭─ ⚠️ 免责声明 ───────────────────────────────────────────────╮
│ 1. 本项目仅供学习、研究和个人自托管测试。                    │
│ 2. 不得用于任何违反当地法律法规、服务条款或第三方权益的用途。 │
│ 3. 使用者需要自行承担部署、运营、数据、网络和合规风险。       │
│ 4. 安装前请确认你拥有当前服务器的管理权限和操作授权。         │
│ 5. 商业部署、再分发、托管运营或二次销售前请先确认授权条款。   │
╰────────────────────────────────────────────────────────────╯
...
EOF
  ...
  local answer=""
  read -r -p "请输入 yes 表示你已阅读并接受以上免责声明与安装范围： " answer
  [[ "${answer}" == "yes" ]] || die "你未接受免责声明与安装范围，脚本已退出。"
}
...
function requireFileEntryPathMatches(entry, fileName, label, acceptedPaths = []) {
  ...
  const recordedPath = path.resolve(entry.path);
  const acceptedPathSet = new Set(
    acceptedPaths
      .filter((acceptedPath) => typeof acceptedPath === 'string' && acceptedPath.trim() !== '')
      .map((acceptedPath) => path.resolve(acceptedPath))
  );
  if (!acceptedPathSet.has(recordedPath)) {
    fail(`${label}.path 与当前证据包或记录的 bundleDirectory 不匹配。`);
  }
}

function verifySummaryFileEntry(bundleDirectory, entry, fileName, label, acceptedPaths = []) {
  if (!entry || typeof entry !== 'object') {
    fail(`${label} 缺少文件摘要。`);
  }
  requireFileEntryPathMatches(entry, fileName, label, acceptedPaths);
...
function verifyAgentEvidenceFileEntry(bundleDirectory, entry, relativePath, label, acceptedPaths = []) {
  if (!entry || typeof entry !== 'object') {
    fail(`${label} 缺少文件摘要。`);
  }
  requireFileEntryPathMatches(entry, path.basename(relativePath), label, acceptedPaths);
  if (!Number.isSafeInteger(entry.sizeBytes) || entry.sizeBytes < 0) {
    fail(`${label}.sizeBytes 无效`);
  }
  if (typeof entry.sha256 !== 'string' || !/^[a-f0-9]{64}$/i.test(entry.sha256)) {
...
function bundleFileAcceptedPaths(fileName) {
  const acceptedPaths = [path.join(currentBundleDirectory, fileName)];
  if (manifestBundleDirectory !== '') {
    acceptedPaths.push(path.join(manifestBundleDirectory, fileName));
  }
  return acceptedPaths;
}

function bundleRelativeAcceptedPaths(relativePath) {
  const acceptedPaths = [path.join(currentBundleDirectory, relativePath)];
  if (manifestBundleDirectory !== '') {
    acceptedPaths.push(path.join(manifestBundleDirectory, relativePath));
  }
  return acceptedPaths;
}
...
function requireReportTimeRange(report, reportFileName) {
  const startedAt = requireIsoUtcTimestamp(report.startedAt, reportFileName, 'startedAt');
  const completedAt = requireIsoUtcTimestamp(report.completedAt, reportFileName, 'completedAt');
...
if (
  ...
  agentEvidenceManifest.bundles.forEach((bundle, index) => {
    ...
    verifyAgentEvidenceFileEntry(
      ...
      agentManifestPath,
      `${label}.manifest`,
      bundleRelativeAcceptedPaths(agentManifestPath)
    );
    verifyAgentEvidenceFileEntry(
      bundleDirectory,
      ...
      bundleRelativeAcceptedPaths(runtimeSummaryPath)
    );
    if (files.finalSummary) {
      verifyAgentEvidenceFileEntry(
        ...
        finalSummaryPath,
        `${label}.finalSummary`,
        bundleRelativeAcceptedPaths(finalSummaryPath)
      );
    }
    if (files.finalVerifyLog) {
      verifyAgentEvidenceFileEntry(
        ...
        bundleRelativeAcceptedPaths(finalVerifyLogPath)
      );
    }
    ...
    if (files.finalSummary) {
      ...
      if (requirements.agentFinalSummary) {
        ...
        verifyAgentEvidenceFileEntry(
          ...
          [
            path.join(attachedAgentBundleDirectory, 'manifest.json'),
            path.join(currentBundleDirectory, agentManifestPath)
          ]
        );
        verifyAgentEvidenceFileEntry(
          bundleDirectory,
          ...
          [
            path.join(attachedAgentBundleDirectory, 'final-acceptance-verify.txt'),
            path.join(currentBundleDirectory, finalVerifyLogPath)
          ]
        );
...
if (requirements.runtimeEvidence) {
  ...
  const csrfCheck = findReportCheck(smokeReport, 'CSRF rejection probe');
  if (!csrfCheck) {
    fail('要求 runtime 现场证据，但 smoke-report.json 缺少 CSRF rejection probe。');
  }
  if (csrfCheck.httpStatus !== 403) {
    fail(`要求 runtime 现场证据，但 CSRF rejection probe httpStatus=${csrfCheck.httpStatus ?? 'missing'}`);
  }
  if (csrfCheck.errorCode !== 'csrf.required') {
    fail(`要求 runtime 现场证据，但 CSRF rejection probe errorCode=${csrfCheck.errorCode ?? 'missing'}`);
  }
...
if (requirements.webhookSmoke) {
  ...
  const webhookReportTimeRange = requireReportTimeRange(webhookReport, 'webhook-smoke-report.json');
  ...
  webhookReport.targets.forEach((target, index) => {
    const targetLabel = `webhook-smoke-report.json targets[${index}]`;
    if (!target || typeof target !== 'object' || Array.isArray(target)) {
      fail(`要求 webhook 烟测证据，但 ${targetLabel} 不是有效对象。`);
    }
    const checkedAt = requireIsoUtcTimestamp(target.checkedAt, targetLabel, 'checkedAt');
    if (checkedAt < webhookReportTimeRange.startedAt) {
      fail(`${targetLabel}.checkedAt 早于 startedAt。`);
    }
    if (checkedAt > webhookReportTimeRange.completedAt) {
      fail(`${targetLabel}.checkedAt 晚于 completedAt。`);
    }
    if (target?.status !== 'passed') {
      fail('要求 webhook 烟测证据，但 webhook-smoke-report.json 存在未通过目标。');
    }
    if (
      !Number.isSafeInteger(target.httpStatus) ||
      target.httpStatus < 200 ||
      target.httpStatus > 299
    ) {
      fail(`${targetLabel}.httpStatus=${target.httpStatus ?? 'missing'}`);
    }
    if (!Number.isSafeInteger(target.responseBytes) || target.responseBytes < 0) {
      fail(`${targetLabel}.responseBytes=${target.responseBytes ?? 'missing'}`);
    }
...
if (requirements.finalSummary) {
  ...
  verifySummaryFileEntry(
    bundleDirectory,
    finalSummary.manifest,
    'manifest.json',
    'final summary manifest',
    bundleFileAcceptedPaths('manifest.json')
  );
  verifySummaryFileEntry(
    bundleDirectory,
    ...
    bundleFileAcceptedPaths('final-acceptance-verify.txt')
  );
...
if (requirements.releaseSummary) {
  ...
  verifySummaryFileEntry(
    ...
    bundleFileAcceptedPaths('final-acceptance-summary.json')
  );
  verifySummaryFileEntry(
    bundleDirectory,
    ...
    bundleFileAcceptedPaths('release-acceptance-verify.txt')
  );
...
常用参数:
  ...
  --require-runtime-evidence
                      要求报告中存在完整 runtime summary、audit.valid=true、无超限/禁用配额策略、具备 Xray/端口转发能力的 Agent session、Xray inbound、端口转发规则，且无 critical 告警/命令死信
...
校验参数:
  --require-runtime-evidence     要求 manifest.createdAt 为有效 UTC ISO 时间、manifest.bundleDirectory 非空、主 manifest 证据路径匹配，smoke-report.json schemaVersion/status/startedAt/completedAt/runtimeEvidenceRequired 有效且 completedAt 不早于 startedAt，checks 非空、具名、checkedAt 位于 startedAt/completedAt 窗口内且全部 passed，CSRF rejection probe 记录 403 csrf.required，runtime acceptance summary 记录 required=true、完整结构、audit.valid=true、无超限/禁用配额策略、具备 Xray/端口转发能力的 Agent session，并满足 Agent/Xray/端口转发现场门槛
