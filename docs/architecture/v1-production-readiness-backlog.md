...
- The HTTP adapter can enforce optional bootstrap bearer tokens and signed HttpOnly operator sessions for operator mutations and sensitive reads, and protected operator-route authentication failures now write sanitized denied audit evidence with per-source failure throttling. Installer nginx templates verify the browser session with `auth_request` before injecting the backend token, but this is still not a full identity platform.
