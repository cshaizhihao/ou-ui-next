...
- `npm run test -- production-webhook-smoke-script install-master-script` - 2 files / 45 tests
- `npm run test -- agent-install-script` - 1 file / 17 tests
- `npm run test -- install-master-script` - 1 file / 42 tests
...
- `npm run lint`
- `npm run test` - 61 files / 712 tests
- `npm run build`
- `git diff --check`
...
- External notification smoke output from operator-provided Telegram/webhook endpoints with real credentials. The repository now has reusable `ou ns` / `npm run smoke:notifications` tooling, `ou ws` / `npm run smoke:webhooks` webhook tooling, and optional `ou qa --include-notification-smoke` bundle capture for Telegram, but this audit still lacks a real provider run proving delivery outside automated tests.
