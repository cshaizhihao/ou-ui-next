...
The installed CLI also exposes `ou-ui acceptance` / `ou qa`, which writes a timestamped evidence bundle under `/var/lib/ou-ui-next/acceptance/` containing doctor output, HTTP smoke terminal output, browser smoke terminal output, notification/webhook/archive-smoke skipped or executed evidence, sanitized JSON reports, a `browser-screenshots.tar.gz` archive, and a manifest with per-file byte sizes and SHA-256 hashes for later archive verification. `ou d` / the bundle `doctor.txt` now reports browser-smoke readiness too: presence of `scripts/production-browser-smoke.cjs`, Playwright module availability, and whether the Chromium executable exists. The same CLI exposes `ou-ui acceptance-verify` / `ou qv` to re-check those hashes after archiving or transferring the bundle; the verifier remains backward-compatible with older bundles that only contain the original doctor/smoke/report files or pre-webhook browser/notification evidence. For archived field evidence, `ou qv` can add strict report-content gates with `--require-runtime-evidence`, `--require-browser-smoke`, `--require-notification-smoke`, and `--require-webhook-smoke`, failing archived bundles whose runtime summary is incomplete, browser smoke was skipped/failed, browser screenshots were disabled/missing, Telegram notification smoke was skipped/failed, or webhook smoke was skipped/failed/missing sanitized target URLs. Bundles created with `ou qa --include-archive-smoke` can additionally use `ou qv --require-archive-smoke`, which fails if archive smoke was skipped/failed or the report is missing sanitized sink evidence. Bundles produced by `ou qf` can also be rechecked with `ou qv --require-final-summary`, which verifies `final-acceptance-summary.json` schema/status, strict gate markers, and the recorded manifest/transcript size/SHA-256 against the archived files; `ou qvf` is the one-command wrapper that applies runtime, browser, notification, webhook, and final-summary strict gates together. `ou-ui final-acceptance` / `ou qf` wraps the final field sequence into one command: it generates the bundle with runtime, Telegram, and webhook evidence collection enabled, leaves archive smoke skipped unless `--include-archive-smoke` is explicitly passed, rejects `--skip-browser-smoke`, requires an explicit Telegram target, immediately runs the same strict `ou qv` gates against the bundle it just wrote, saves that strict verifier transcript as `final-acceptance-verify.txt`, and writes `final-acceptance-summary.json` with the manifest and verifier transcript size/SHA-256. To keep the bundle self-consistent, `ou qa` fixes the installed panel URL, root-only credentials file, backend env file, bundle-local report paths, screenshot directory, notification report path, webhook report path, and archive report path, while still allowing read-only smoke tuning flags such as `--timeout-ms`, `--insecure-tls`, `--skip-csrf-probe`, and `--require-runtime-evidence`; low-resource hosts can explicitly use `--skip-browser-smoke`, which records the skip in the manifest rather than pretending browser acceptance ran. Notification smoke and webhook smoke are skipped by default to avoid external side effects during every acceptance run; operators can opt in with `--include-notification-smoke --telegram-admin-chat-id <id>` or `--include-notification-smoke --telegram-binding-id <id>` for Telegram, and `--include-webhook-smoke` plus optional `--webhook-url` / `--webhook-urls` / `--webhook-bearer-token` / `--webhook-bearer-token-file` for real webhook delivery. Archive smoke is also skipped by default; operators can opt in with `--include-archive-smoke`, then recheck the archived evidence with `ou qv --require-archive-smoke` and save provider-side receipt evidence for real external archive acceptance.
...
- `npm run test -- production-archive-smoke-script archive-sink audit-anchor-sink install-master-script` - 4 files / 70 tests
...
- `npm run test -- install-master-script` - 1 file / 46 tests
...
- `npm run lint`
- `npm run test` - 62 files / 721 tests
- `npm run build`
- `git diff --check`
