...
| Audit chain and denied evidence | `src/server/control-plane/operator-session-store.test.ts`, `src/server/control-plane/file-control-plane-repository.test.ts`, `src/server/control-plane/audit-anchor-sink.test.ts`, `src/server/control-plane/archive-sink.test.ts`, `src/server/control-plane/object-storage-sink.ts`, `src/services/api/http-control-plane-client.test.ts`, `src/services/api/service-backed-control-plane-api.test.ts` | Covered for session audit, denied audit, chain verification, duplicate audit rejection, exported verification, and external audit/archive sinks across local JSONL, webhook, and S3-compatible object storage with egress hardening; still needs real provider delivery evidence and immutable retention policy proof. |
...
- `npm run lint`
- `npm run test` - 61 files / 716 tests
- `npm run build`
- `git diff --check`
