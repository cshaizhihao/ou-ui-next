...
- External subscription source sync now restricts remote fetches to `http` / `https`, applies per-source request timeout and response body limits before parsing, and falls back to service defaults when a source does not override them. Unsupported protocols, timeouts, oversized responses, and upstream failures are projected as failed sync results and audit-chain entries.
