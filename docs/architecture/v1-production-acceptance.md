...
- External subscription source sync must reject unsupported URL protocols and localhost/private/local IP targets before remote fetch, enforce per-source request timeout and response size limits, and record the outcome as sync failure state plus audit-chain evidence.
