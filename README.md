...
默认 `ou qv` 只校验 manifest 中记录的文件大小和 SHA-256，兼容旧证据包；需要把归档后的证据包作为现场验收门槛时，可以追加强制检查，要求 manifest.bundleDirectory 非空，且 runtime summary、浏览器烟测、浏览器截图归档、通知烟测和 webhook 烟测都已真实通过；如果证据包是用 `ou qa --include-archive-smoke` 显式纳入真实外部归档投递生成的，还可以追加 `--require-archive-smoke`；如果证据包附带了 provider 侧回执文件，可追加 `--require-external-receipts` 要求至少一个回执附件存在且哈希匹配；如果回执包含 `ou-ui-next.archive-provider-evidence.v1` 脱敏 JSON，可追加 `--require-archive-provider-evidence` 要求至少一个回执证明对象存储投递和 provider 侧 Object Lock/retention 策略；如果回执包含 `ou-ui-next.timestamp-evidence.v1` 脱敏 JSON，可追加 `--require-timestamp-evidence` 要求至少一个回执证明第三方时间戳 receipt 已脱敏、已验证且 hash 匹配；如果证据包附带了干净服务器安装证据，可追加 `--require-clean-install-evidence` 要求至少一个安装 summary 符合 `ou-ui-next.clean-install-evidence.v1` 且哈希匹配；如果证据包附带了 Agent 主机证据，可追加 `--require-agent-evidence` 要求至少一个 Agent 证据包存在、Agent manifest.bundleDirectory 非空、关键文件哈希匹配，并且 `runtime-summary.json` 证明 Xray inbound、端口转发 service、pending queue 和 guardrail 状态满足现场门槛；若要把 Agent 主机证据作为生产发布证据，还可追加 `--require-agent-final-summary`，要求 Agent 包包含 `ou-agent qf` 生成的 `final-acceptance-summary.json` 非空 bundleDirectory 和校验 transcript，并复核其中记录的 manifest/transcript 哈希；如果证据包来自 `ou qf`，还可以用 `--require-final-summary` 复核 Master 侧 `final-acceptance-summary.json` 的非空 bundleDirectory 与严格校验 transcript 是否匹配；如果证据包来自 `ou qfa`，可用 `--require-release-summary` 复核 `release-acceptance-summary.json` 的非空 bundleDirectory、发布复核 transcript 哈希和全量 gate 标记：
