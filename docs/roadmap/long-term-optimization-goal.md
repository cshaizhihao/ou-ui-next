...
- Forwarding automatic quota guardrail derivation and quota policy projections now use billed forwarding bytes to detect rule overage even when `quotaExceeded` is omitted, and generated pause/resume metadata carries the same billed usage evidence shown by rule/account/tunnel quota rows.
