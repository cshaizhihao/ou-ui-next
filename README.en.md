...
- **Service-backed HTTP control plane**
  ...
  - protected `/metrics` exposes the current diagnostics snapshot as Prometheus text gauges for external scraping and renders task completion, per-operation completion, runtime apply, command ACK, and command result latency as `_bucket` / `_sum` / `_count` histograms
