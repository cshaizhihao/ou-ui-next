...
- Accepted `agent.delete` tasks revoke every active runtime credential for the deleted Agent inside the same service transaction and append `agent.credential.revoked` audit records, so a removed host cannot keep authenticating with a previously issued runtime token.
