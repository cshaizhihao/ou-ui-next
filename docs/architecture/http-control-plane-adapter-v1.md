...
- `/events/v1/tasks` now supports cursor-resumable snapshots with the `cursor` query parameter or standard `Last-Event-ID` header before continuing with durable task/audit read-model tailing; `/events/v1/system-alerts` uses the same cursor pattern for active system-alert snapshots, emits a new snapshot when the derived alert fingerprint changes, and reconciles active alerts against a persisted active/resolved lifecycle record. The installer-generated Nginx panel proxy keeps `/events/v1/*` unbuffered and explicitly returns `text/event-stream`.
