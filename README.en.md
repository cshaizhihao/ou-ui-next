...
- **Service-backed HTTP control plane**
  ...
  - protected `/events/v1/system-alerts` streams the current active system-alert snapshot and emits a new snapshot when the alert fingerprint changes; active alerts are reconciled with a durable lifecycle read model and persist `active` / `resolved` evidence in the control-plane store, and the default sqlite-backed production deployment now follows alert snapshots across panel instances; when `OU_UI_SYSTEM_ALERT_WEBHOOK_URL` is configured, alert activation, meaningful updates, and resolution send sanitized JSON webhook notifications with sanitized structured delivery logs
