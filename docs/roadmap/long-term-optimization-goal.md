...
- Added shared-inbound Xray clients now also enqueue a traceable `subscription.generate` task from the same operator flow, using stable subscription identity, secure path preview, and all supported public output formats. Non-primary client subscription delete/rotation lifecycle remains a follow-up item.
