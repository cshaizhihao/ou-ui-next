...
The bearer-token layer is a hardening slice, not the final identity platform. Production V1 still needs password/session or OIDC/JWT operator identity, Agent registration and rotation, hashed secret storage, secret revocation, rate limiting, and audit-visible login/token lifecycle events.
