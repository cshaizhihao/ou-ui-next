...
- `/api/v1/observability-metrics` returns a protected operator diagnostics snapshot derived from current tasks, command outbox, Agent liveness, active system alerts, and audit-chain verification. It covers task status totals, command backlog/lease/overdue/dead-letter counts, Agent offline/degraded counts, alert severity counts, and audit validity.
