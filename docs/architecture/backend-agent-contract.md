...
- [ ] Agent 上报 ingress/egress counters，Master 聚合并标记采样 gap。
...
- [ ] 有数据库迁移、备份、恢复演练。
- [ ] 有灰度发布和 Agent 版本兼容矩阵。
- [ ] 有生产告警：Agent offline、quota exceeded、reload failed、audit write failed、command backlog（quota exceeded 与 command outbox overdue/dead-letter 已进入系统告警 lifecycle）。
