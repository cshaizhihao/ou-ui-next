...
type TelegramBotSettings = {
  ...
  schedules: Array<{
    id: string;
    kind: 'traffic_threshold_scan' | 'expiry_scan' | 'subscription_update_scan' | 'provider_sync_scan' | 'system_alert_scan' | 'daily_report' | 'weekly_report' | 'delivery_retry';
...
- The background schedule scanner generates `traffic.threshold`, `subscription.expiring`, and `subscription.updated` delivery records from customer, subscription-user, Xray-client, forwarding-owner, and forwarding-rule bindings; `provider.sync_warning` / `provider.sync_failed` delivery records from subscription source sync alerts; and `system.alert` delivery records for configured administrator chat IDs. It does not call Telegram directly; the retry sweep sends the queued records through the same persisted delivery path as manual retries.
