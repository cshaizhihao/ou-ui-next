...
- **Service-backed HTTP control plane**
  ...
  - protected `/events/v1/system-alerts` streams the current active system-alert snapshot and emits a new snapshot when the alert fingerprint changes; active alerts are reconciled with a durable lifecycle read model and persist `active` / `resolved` evidence in the control-plane store, and the default sqlite-backed production deployment now follows alert snapshots across panel instances; external notification fan-out remains a production-hardening item
  ...
  - Agent runtime log chunks are retrievable through the protected read API and pruned by the effective retention policy, defaulting to 7 days and 5000 chunks per Agent; `/api/v1/agent-log-retention-policy`, snapshots, and the execution workspace surface the active policy beside retained runtime logs, task IDs, command IDs, and streams
  ...
  - Agent telemetry now reports CPU, memory, disk, Linux load averages, network traffic, and managed systemd service health for the Agent, Xray, and port-forwarding units; managed-host details surface the load and service-health state, and required service failures enter system alerts instead of requiring operators to inspect host files
  ...
  - operator bearer authentication failures on protected REST, SSE, and Prometheus routes append sanitized `audit.denied` evidence without bearer tokens; repeated failures from the same source are throttled by a default 60-second / 20-failure window, return `429 operator_auth.rate_limited` after the limit, and append only one throttle audit entry per window
