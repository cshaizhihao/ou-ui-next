...
- External subscription source sync now restricts remote fetches to `http` / `https`, applies a request timeout, and caps response body size before parsing. Unsupported protocols, timeouts, oversized responses, and upstream failures are projected as failed sync results and audit-chain entries.
