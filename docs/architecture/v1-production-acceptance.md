...
- Runtime apply failures caused by post-apply health checks must create a system actor rollback task from the failed command snapshot, dispatch it to the same Agent, and link the failed source task through `rollbackTaskId`; preflight-style artifact/schema failures must not trigger that health rollback path.
