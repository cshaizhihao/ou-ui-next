...
- The HTTP adapter can enforce optional bootstrap bearer tokens for operator mutations, sensitive reads, Agent poll, and Agent events, and protected operator-route authentication failures now write sanitized denied audit evidence with per-source failure throttling. This prevents trusting spoofed `X-Actor`/group headers when auth is configured and caps obvious audit-flood attempts, but it is not a full identity platform.
- Bootstrap tokens are configured from environment variables and are not yet durable, hashed at rest, rotated, revoked through the panel, or audited as login/token lifecycle events.
