...
- Service-backed external subscription sync now restricts remote fetches to `http` / `https`, blocks localhost and private/local IP literals plus DNS-resolved private/local targets, pins the default production request to the verified public DNS address while preserving the original Host / HTTPS SNI, enforces an optional host egress allowlist before remote read, applies per-source request timeout and response body limits, and writes allowlist-miss/blocked-target/timeout/oversize/unsupported-protocol outcomes into the sync failure state and audit hash chain. Production still needs distributed fetch rate limiting.
