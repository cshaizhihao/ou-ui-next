...
- Service-backed external subscription sync now restricts remote fetches to `http` / `https`, blocks localhost and private/local IP literals plus DNS-resolved private/local targets before calling the fetcher, applies per-source request timeout and response body limits, and writes blocked-target/timeout/oversize/unsupported-protocol outcomes into the sync failure state and audit hash chain. Production still needs operator-configurable egress allowlists, DNS rebinding-resistant connection pinning, and distributed fetch rate limiting.
