...
- Service-backed external subscription sync now restricts remote fetches to `http` / `https`, applies per-source request timeout and response body limits, and writes timeout/oversize/unsupported-protocol outcomes into the sync failure state and audit hash chain. Production still needs network egress allowlists and distributed fetch rate limiting.
