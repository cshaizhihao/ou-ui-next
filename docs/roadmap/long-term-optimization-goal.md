...
- Forwarding rule rows now also expose copyable runtime evidence for ready rules, using the same binding/service/quota/guardrail/blocked-control package as recovery contexts so healthy and unhealthy rules have a consistent support handoff.
...
- Subscription source rows and matched-source cards now open a focused sync-diagnosis drawer that translates structured sync warnings into operator-facing causes and next actions, includes fetch budget / remote / source-rule context, and copies a diagnostic package with raw warning evidence for support without showing raw codes in the UI.
- Subscription source sync diagnosis now derives an import/conversion impact summary from real sync warnings, separating blocked deliverability impact from expected filter/dedupe review impact and including the impact next action in copied diagnostics.
...
- Customer-node create/edit drawers now front-load a `Save Preflight` summary before the form basics, add a first-class `Security Material` readiness gate for TLS/Reality state, and block missing active Reality SNI/public/private key material before save instead of silently regenerating it during submit.
...
- Subscription link drawers now start with a delivery-status brief that derives ready / attention / blocked state from enabled state, quota, expiry, runtime guardrails, generated nodes, output formats, request limits, and public portal URLs; the same verdict is included in copied subscription diagnostics so operators can decide whether a customer link is safe to deliver before opening every format panel.
- Subscription link drawers now include an executable delivery check that requests the customer portal plus every selected public output format, reads HTTP status, content type, `subscription-userinfo`, generated-node, conversion, and producer headers, then displays and copies a task-scoped delivery diagnosis instead of relying only on static link previews.
- Subscription delivery checks now surface a contextual recovery panel when requests fail, nodes are missing, conversion warnings appear, or related sources have sync issues; operators can jump directly to matched inventory, matched nodes, or the relevant source sync-diagnosis drawer from the failed delivery context.
- Subscription link drawers now copy a customer delivery package that combines static delivery readiness, portal/output links, quota/expiry/request-limit/guardrail evidence, and the latest executable delivery-check result when available; unchecked packages explicitly say the delivery check has not run.
- Subscription link drawer guardrail status now derives expired and over-quota policy state from `expiresAt` and traffic usage even when `runtimeDisabledByPolicy` / `quotaExceeded` flags are missing, keeping delivery brief, diagnostics, and copied packages consistent.
- Subscription identity rows now expose a copyable delete audit package before destructive removal, preserving identity, delivery, guardrail, public-link, token-preview, secure-path, and generated-node impact evidence without triggering deletion.
...
- Service-backed Agent heartbeat and telemetry ingestion now samples routine raw event persistence with `OU_UI_AGENT_EVENT_HIGH_FREQUENCY_PERSIST_EVERY`; sampled events refresh durable session / rollup evidence, while cached unsampled high-frequency events are accepted through an in-process sequence cache without entering the SQLite JSON-state transaction path.
- Service-backed Agent command polling now keeps a pollable-command hot cache, so warm-session empty polls return without opening a SQLite JSON-state transaction while pending/dispatched command leases still use the durable outbox path.
- Service-backed Agent command log chunk de-duplication now uses a command/chunk keyed transaction lookup instead of cloning and scanning the full Agent event list, reducing Xray apply smoke pressure while preserving command log retention and runtime evidence semantics.
- Service-backed Agent command log chunk persistence can now be sampled with `OU_UI_AGENT_LOG_CHUNK_PERSIST_EVERY`; ACK/result/config revision/preflight/snapshot evidence remains fully durable, while high-volume command stdout/stderr chunks can avoid rewriting the large SQLite JSON-state payload on every chunk.
- SQLite transactions now track whether a repository mutation actually occurred and skip the large `json-state-v1` payload rewrite for read-only / unsampled routine Agent event transactions.
- Existing schema-v2 SQLite databases now open without reparsing and rewriting the full compatible state row or rebuilding `control_plane_entity_index`; full index rebuild is reserved for new state creation or newly applied index migrations.
- HTTP Agent routine poll / heartbeat / telemetry console logs now support `OU_UI_CONTROL_PLANE_AGENT_ROUTINE_LOG_SAMPLE_EVERY`, reducing always-on real-Agent log churn while preserving command polls, ACK/result events, failures, denied auth, and runtime evidence logs.
- Control-plane snapshots now summarize runtime config revision artifacts to the runtime evidence fields needed by the UI while preserving full artifacts through the dedicated config-revision APIs and persistent store, reducing real-Agent dashboard payload weight without weakening release-evidence traceability.
- Service-backed Control Plane snapshots now hydrate read models and assemble dashboard evidence from one repository state snapshot instead of replaying many SQLite JSON-state reads, reducing real-Agent dashboard CPU/read pressure while keeping command payloads, runtime artifact bodies, and Agent credential hashes out of the snapshot response.
- Service-backed Control Plane snapshots now use a lightweight repository `state_revision` plus local read-model/time-bucket cache, so unchanged dashboard polls avoid reparsing and rehydrating the full SQLite JSON state while still invalidating on persisted writes and task read-model changes, with high-frequency Agent liveness/telemetry refreshed through short time buckets.
- HTTP Agent endpoints now throttle repeated authentication failures per source/token with `OU_UI_CONTROL_PLANE_AGENT_AUTH_FAILURE_*`, returning `agent_auth.rate_limited` and capping denied-audit writes so stale Agent tokens cannot flood the Control Plane state/log path.
- Runtime Agent token success resolution now uses a short tokenHash-keyed identity cache plus `lastUsedAt` write throttling via `OU_UI_CONTROL_PLANE_AGENT_AUTH_SUCCESS_CACHE_TTL_MS` and `OU_UI_CONTROL_PLANE_AGENT_CREDENTIAL_LAST_USED_PERSIST_INTERVAL_MS`, reducing normal poll / heartbeat / telemetry credential read-write churn without storing raw Agent tokens.
- Routine heartbeat / telemetry ingestion now reuses capabilities carried by the heartbeat or existing Agent session before falling back to runtime credential lookup, avoiding unnecessary credential-list reads on the hot event path.
...
- Agent result handling now validates runtime release evidence correlation before marking apply work successful: config revision, preflight plan, runtime snapshot, task, target, Agent, and module must match the same command chain, otherwise the result is failed with `runtime_release.correlation_mismatch` instead of writing false verified evidence.
...
- `smoke:xray-client-action` now extends the same real-Agent Xray diagnostic to add and delete a client through `/api/v1/xray-client-actions`, verifying task, command outbox, config revision, preflight, runtime snapshot, runtime diagnosis, and client counter evidence for each client action.
- The Xray production smoke now performs a default final `inbound.delete` cleanup with high-risk confirmation, verifies `remove_inbound` Agent evidence, and treats the latest remove revision as freeing the smoke listen port for future diagnostics.
- The Xray production smoke now also fails when command outbox, config revision, preflight plan, or runtime snapshot evidence is not correlated to the same task, target inbound, and Agent, preventing a green smoke report from being produced by unrelated runtime artifacts.
- The Xray production smoke now also verifies customer-node read-model consistency for the same runtime task: create/update/client add/delete phases must expose Agent-result `runtimeDeployment` proof on `/api/v1/inbounds`, while cleanup delete must remove the target inbound from the read model and redacted phase report.
- The Xray production smoke failure path now preserves a redacted last-observed runtime evidence report with task, command timing, config revision, preflight, snapshot, Agent log chunk, read-model, and evidence-gap summaries; command result timeout defaults now leave a longer window for Xray preflight plus systemd apply before marking an Agent command dead-lettered.
- The Agent runtime installer now adds an Agent-side command timeout guard plus live command progress log chunks for Xray apply/remove phases; command results are delivered before best-effort bulk log chunks, and the Control Plane preserves agent/runtime/result diagnostic chunks even when stdout/stderr chunk sampling is enabled, so long-running or stuck runtime applies can surface command progress or fail with Agent evidence instead of remaining silent after ACK until the Control Plane result timeout.
- Runtime command timeout evidence now distinguishes ACK-less dispatch failures from ACKed commands that never return an Agent result, preserving `agent-result-missing-after-ack` operator-action diagnostics in config revision health summaries and customer-node/task runtime evidence so Xray cleanup/apply failures do not collapse into a generic deadline message.
- Customer-node runtime evidence drawers now turn `agent-result-missing-after-ack` diagnostics into an Agent recovery path with copyable, audit-backed recovery commands that use the current Master `/install/ou-agent.sh` URL, plus optional remote upgrade for Agents advertising self-update, so operators can recover the host directly from the failed Xray cleanup/apply context.
...
- Task runtime verification now surfaces concrete Agent/config/preflight failure reasons directly inside the failed verification steps and copied runtime context packages, so operators can see why an Xray apply failed without hunting through separate raw task fields.
...
- Xray inbound delete read models now keep the customer node visible as `applying` or `error` until `inbound.delete` has verified Agent runtime evidence, preventing failed delete commands such as ACK timeouts from disappearing as if runtime cleanup had succeeded.
- Customer-node runtime evidence now focuses the latest failed or pending `inbound.delete` cleanup task over older verified apply proof, and the diagnosis drawer can resubmit the existing typed delete-client/runtime-cleanup flow while keeping command, preflight, snapshot, and next-action evidence scoped to the failed cleanup.
...
- Round 5 now passes live `SubscriptionClientIdentity` data into the Xray client drawer and adds a per-client subscription delivery panel with bound/preview/blocked state, secure path, portal URL, all generated output links, and safe copyable delivery diagnostics.
- Round 5 delivery diagnostics intentionally omit client credentials, subscription token hashes, raw runtime payloads, and runtime snapshot bodies while preserving enough identity, policy, and task evidence for operator handoff.
- Round 6 splits Xray apply eligibility from telemetry freshness: degraded Agents with explicit active command-channel and Xray service evidence remain eligible for real apply/smoke, while telemetry sampling gaps stay visible as warnings.
- Round 6 production Xray smoke and customer-node save preflight now share the same apply-eligible semantics and explain blocked Agents with service/policy evidence instead of requiring `online` status alone.
- Round 6 production Xray smoke now fails closed when stale smoke inbounds, active inbound runtime tasks, or pending Agent command outbox items would pollute evidence, and its diagnostics include API method/path timeout context plus dirty-state summaries.
- Round 6 live 4174 validation verified `inbound.create` Agent result evidence on `agent-fb073c1d5053`; later phases are intentionally blocked until stale `xray-live-smoke-*` residue is cleaned up or explicitly diagnosed with `--allow-dirty-smoke-state`.
- Round 7 passes sanitized Agent runtime credential summaries into the node workspace and uses them to gate Agent recovery command UI, so manual recovery is only actionable when an active runtime credential exists.
- Round 7 aligns Agent recovery UI with Control Plane capability rules: one-click remote upgrade is only offered for Agents advertising `self-update`, while legacy Agents show explicit manual-command or blocked-state guidance.
- Round 7 updates ACK-silent customer-node runtime evidence drawers to show the same Agent recovery readiness model, including missing runtime credential, missing command preview API, missing `self-update`, and command-channel warning copy.
- Round 8 adds a copyable subscription delivery package to the customer link drawer, merging delivery readiness, public links, quota/expiry/request-limit state, guardrail evidence, and executable check results into one customer/support handoff artifact.
- Round 9 adds an import/conversion impact section to source sync diagnosis, translating unsupported/invalid/filter/dedupe/sync-failure evidence into operator-facing deliverability impact and copied support context.
- Round 10 adds a copyable forwarding runtime evidence package to ready rule evidence cards, extending the existing failed-rule recovery context into a consistent support artifact for every forwarding/tunnel state.
- Round 11 aligns subscription delivery guardrail display with derived quota/expiry evidence, preventing expired or over-quota identities from being copied as `active` when explicit runtime policy flags are missing.
...
- Customer-node contextual quick actions now surface the accepted typed client-action task directly in the sticky action bar, including Agent-result verification state, runtime task id, evidence stage, diagnostic copy, release-evidence workspace, and rollback entry points when available, so list-level actions no longer disappear into the background after submission.
- Customer-node bulk contextual actions now keep a scoped task feedback queue in the sticky action bar, showing each selected node's accepted typed client-action task, runtime task id, Agent-result stage, and verified / waiting / failed counts so multi-node renew, quota, enable/disable, and reset operations remain traceable after confirmation.
- Customer-node create/edit saves now return the accepted inbound runtime task and linked subscription task from `AppShell` to `NodesPage`, then show a save feedback bar with task id, Agent-result evidence stage, task/command/config/preflight/snapshot steps, diagnostic copy, release-evidence workspace, node evidence, and rollback entry points so save actions do not disappear after the drawer closes.
- Customer-node contextual add-traffic, renew, enable/disable, and used-traffic reset actions now open an inline action confirmation composer before submitting, with editable GB/day values, impact metrics, runtime target/evidence labels, and a single confirm path that still submits through the typed Xray client action pipeline and its runtime evidence feedback.
