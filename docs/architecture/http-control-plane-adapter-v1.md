...
- Operator bearer authentication failures on protected REST, SSE, and Prometheus routes return `401 unauthorized` promptly and append sanitized `audit.denied` evidence without bearer token material, with per-source throttling to prevent unbounded audit-chain growth. Denied-audit appends read the previous audit hash through the active repository transaction, so sqlite-backed production deployments do not self-block on the serialized repository queue while handling auth failures.
