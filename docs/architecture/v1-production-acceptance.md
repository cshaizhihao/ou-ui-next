...
- The execution workspace must expose retained Agent runtime log chunks with Agent, task, command, stream, timestamp context, and the current retention policy so operators can inspect real runtime output without reading local state files; when retention pruning removes raw chunks, it must also expose queryable/exportable UTC-day archive summaries with counts, bytes, sessions, time ranges, and content hashes for the pruned evidence.
