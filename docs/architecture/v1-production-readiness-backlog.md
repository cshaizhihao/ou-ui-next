...
- `/api/v1/observability-metrics` now exposes task state totals, command outbox backlog/lease/overdue/dead-letter counts, Agent offline/degraded counts, active alert severity counts, and audit-chain verification state. Production still needs ACK/result/apply latency histograms, rollback counters, quota-exceeded metrics, audit write-failure counters, and external metrics scraping.
