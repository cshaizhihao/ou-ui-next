...
- **Service-backed HTTP control plane**
  ...
  - operator bearer authentication failures on protected REST, SSE, and Prometheus routes return `401 unauthorized` promptly and append sanitized `audit.denied` evidence without bearer tokens; repeated failures from the same source are throttled by a default 60-second / 20-failure window, return `429 operator_auth.rate_limited` after the limit, and append only one throttle audit entry per window; `ou-ui doctor` reports operator auth throttle window and failure-limit config health before backend startup; sqlite-backed denied-audit writes read the previous audit hash through the same transaction so auth failures cannot self-block on the repository queue
