...
- Customer-node rows now expose a focused inbound clients drawer backed by the live `XrayInbound.clients[]` read model. Operators can inspect every client under the inbound, copy a per-client share URI, add a runtime client, and run per-client enable/disable, renewal, traffic increase, used-traffic reset, and delete actions through the explicit Xray client action API. Subscription identity lifecycle management for non-primary clients remains a follow-up item.
