...
- SQLite persistence now compacts high-frequency Agent heartbeat and telemetry events before writing the compatible JSON state row and uses compact JSON serialization, reducing real Agent event CPU/write pressure while preserving command result, log chunk, task, preflight, revision, and snapshot evidence.
